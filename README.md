...
**3. Describe your truck.** Edit `AS_BUILT_CONFIGURATION.md` — serials,
deviations from stock, modifications. This file is injected into every
diagnosis; it's why the AI won't quote stock procedures at your modified
truck. (The file in this repo describes the author's truck — replace it
with yours.)
...
The public-domain A0 TM set is fully indexed, but the A1-generation
documentation (IETM, TM 9-2320-392 / -391 series, MWOs, TBs, electrical
schematic packages) was never publicly posted. If you have any of it — disc
image, files, or even photos of paper pages — drop it here (no account
needed, anonymous):

**How to send it — 3 steps, completely anonymous, no accounts:**

1. **Put everything in one folder and zip it.** Whole disc or thumb drive:
   copy ALL of it — every file and folder, including any setup or viewer
   programs. Don't export or print-to-PDF from inside the IETM program (the
   raw files are better). Paper manual? Phone photos work: straight-on, one
   page per photo, include the cover. Then right-click the folder → Send to
   → Compressed (zipped) folder.
2. **Upload the zip at https://www.swisstransfer.com** (free, up to 50 GB,
   no account). When it finishes it shows you a download link — copy it.
3. **Paste that link into Notepad, save as `link.txt`, and upload that tiny
   file here:** https://www.dropbox.com/request/jhy5o5v056zncbfbku3d
   (also no account). Nobody — including us — learns who you are.

That's it. If you know what the disc/manual is, add a line about it in the
same text file.
